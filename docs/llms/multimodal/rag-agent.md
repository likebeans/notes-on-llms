...
**核心挑战**：维护行列关系，尤其是**合并单元格**（row_span/col_span）

```mermaid
flowchart TB
    DOC[文档页面] --> DET[表格检测<br/>定位边界框]
    DET --> STRUCT[结构识别 - 识别网格线]
    STRUCT --> CELL[单元格定位<br/>row_span/col_span]
    CELL --> OCR[OCR识别 - 单元格内容]
    OCR --> OUT[结构化表格<br/>HTML/DataFrame]
```

**关键能力**

- 明确识别**合并单元格**（row_span/col_span 属性）
- 处理**无边框表格**（通过对齐线索推断）
- 支持**多行表头**（层次化表头结构）
